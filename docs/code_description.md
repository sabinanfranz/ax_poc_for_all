...
## UI
- `ax_agent_factory/app.py`: Streamlit 진입점. 사이드바 입력/버튼(0, 0~1, 0~1~2 실행) → `PipelineManager` 호출 → Stage별 탭 렌더링(Stage 0.1/0.2, 1-A/1-B, 2.1/2.2) 및 로그 expander 출력.

## Core – Pipeline & Research
- `core/pipeline_manager.py`: JobRun 생성(`create_or_get_job_run`) 및 Stage 실행기(`run_stage_0_job_research`, `run_stage_1_ivc`, `run_stage_3_workflow`). Stage 0는 DB 캐시 후 재사용, Stage 1/3은 입력 검증 후 하위 파이프라인 호출.
...
## Core – Workflow & DNA
- `core/workflow.py`: Stage 3 파이프라인. `WorkflowStructPlanner.run`이 `call_workflow_struct`로 `WorkflowPlan`을 받고 디버그 필드를 복사. `WorkflowMermaidRenderer.run`이 `call_workflow_mermaid`로 `MermaidDiagram` 생성. `run_workflow`가 2.1 → 2.2 순서로 호출. 실패 시 예외 전달(스텁은 LLM 헬퍼 내부에서 생성).
...
## Infra
- `infra/db.py`: SQLite 경로 설정(`set_db_path`), 테이블 보장, CRUD(`create_job_run`, `save/get_job_research_result`, `save/get_job_research_collect_result`), LLM 로그 저장/조회(`save_llm_call_log`, `get_llm_calls_by_job_run`).
- `infra/llm_client.py`: Stage별 Gemini 호출/파서/스텁. `call_job_research_collect|summarize`, `call_task_extractor`, `call_phase_classifier`, `call_workflow_struct`, `call_workflow_mermaid`가 공통 sanitizer/JSON 정규화(`_extract_json_from_text`, `_parse_json_candidates`)와 스텁(`_stub_*`)을 사용. `_safe_save_llm_log`로 LLM 호출 메타 저장, `InvalidLLMJsonError` 정의.
...
- `tests/test_ivc_task_extractor.py` / `test_ivc_phase_classifier.py`: sanitizer/파싱/LLM 디버그 필드/스텁 검증.
- `tests/test_ivc_pipeline.py`: Task Extractor → Phase Classifier 연계 및 스텁 동작.
- `tests/test_workflow_struct_mermaid.py`: Workflow Struct/ Mermaid 파서 스텁/정상 JSON 검증.

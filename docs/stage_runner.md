...
4) **JSON 정규화/파싱**: `_extract_json_from_text` → `_parse_json_candidates` 로 코드펜스/서술을 제거하고 여러 후보를 순차 파싱.
5) **검증/변환**: Stage별 `parse_*_dict`로 Pydantic 스키마 변환.
6) **디버그 부착**: `_raw_text`/`_cleaned_json`/`llm_error`를 결과 객체에 `llm_raw_text`/`llm_cleaned_json`/`llm_error`로 복사(UI/테스트용).
7) **저장/전달**: Stage 0.1/0.2는 DB 저장, Stage 1은 메모리 전달. 다음 Stage 입력으로 필요한 최소 필드를 그대로 복사.

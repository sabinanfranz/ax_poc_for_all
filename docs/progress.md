...
| 3. Workflow(2.1 Struct → 2.2 Mermaid) | ✔ 구현(LLM/스텁) | call_workflow_struct / call_workflow_mermaid, UI 탭/테스트 포함 |
...
- Stage 3 Workflow Struct/Mermaid 파이프라인 및 테스트 추가, UI에 2.1/2.2 탭 노출.
- 파서/패턴 문서 추가: `docs/parsing_guide.md`, `docs/stage_runner.md`.
- UI: 모든 탭에 Input/결과/LLM raw/cleaned/error를 노출.

## 다음 스프린트 우선순위
- Stage 1/3 결과(task_atoms/ivc_tasks/workflow_plan/mermaid) 영속화 및 재사용 캐시.

...
- `_generic_llm_json_call(...)`: Workflow Struct/Mermaid 등 Stage 3에서 재사용되는 공통 JSON 호출기(스텁/로깅/파싱 일관화).
...
- **3-A/3-B** (`call_workflow_struct|mermaid`): `_generic_llm_json_call` → `_sanitize_workflow_text` → `_parse_json_candidates` → Pydantic(`WorkflowPlan`/`MermaidDiagram`)로 검증 → 실패/키 없음 시 스텁.
